    ...
    <defs>
      <clipPath id="clip0_1_6236">
        <rect width="190" height="42" fill="white"/>
          ...
          <nav 
            className={`${menuOpen ? 'translate-y-0 opacity-100' : 'translate-y-[-20px] opacity-0 pointer-events-none'} lg:hidden fixed top-[110px] left-0 right-0 mx-4 transition-all duration-200 ease-out bg-black backdrop-blur-3xl rounded-lg border border-white/10 hover-glow z-2000`}
          >
            <div className="flex flex-col gap-4 p-6 z-200">
              {menuItems.map((item, index) => (
                <a
                  key={index}
                  href={item.href}
                  className={`transform transition-all duration-200 ${menuOpen ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}
                  style={{ transitionDelay: `${index * 75}ms` }}
                  onClick={() => setMenuOpen(false)}
                >
                  <div className="text-left text-white font-poppins text-base font-medium py-2 relative group overflow-hidden">
                    <span className="relative z-10 group-hover:text-transparent bg-clip-text bg-gradient-to-r from-[#6a62fd] to-[#4bc3ff] transition-all duration-300">
                      {item.label}
                    </span>
              ...
              <div 
                className={`pt-2 transform transition-all duration-200 ${menuOpen ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}
                style={{ transitionDelay: `${menuItems.length * 75}ms` }}
              >
                <button className="hover-scale button-hover w-full flex px-6 py-4 justify-center items-center gap-2.5 rounded-[10px] border border-white/20 bg-nordek-button-gradient text-white font-poppins text-sm font-medium leading-[18px] transition-all duration-300 hover:bg-opacity-90 hover:shadow-lg hover:border-white/30">
                  Contact Us
                </button>
              </div>
        </div>
