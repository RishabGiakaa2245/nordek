  ...
  return (
    <header className="w-full">
      <div className="w-full max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 pt-8">
        ...
        <div className="relative z-10 flex items-center justify-between px-8 lg:px-[177px] py-6 bg-black/20 backdrop-blur-[12.5px] rounded-[20px] border border-white/10 hover-glow" 
             >
          {/* Logo */}
    ...
    <defs>
      <clipPath id="clip0_1_6236">
        <rect width="190" height="42" fill="white"/>
          ...
          <button 
            className="hover-scale block lg:hidden p-2 text-white transition-transform duration-300 ease-in-out" 
          ...
          <div className="hidden lg:flex items-center gap-9">
            ...
            <nav className="flex items-center gap-[42px]">
              {menuItems.map((item, index) => (
                <a
                  key={index}
                  href={item.href}
                  className="text-white font-poppins text-base font-medium hover:text-white/80 transition-colors relative group"
                >
                  {item.label}
            ...
            {/* Contact Button */}
            <button className="hover-scale flex px-6 py-4 justify-center items-center gap-2.5 rounded-[10px] border border-white/20 bg-nordek-button-gradient text-white font-poppins text-sm font-medium leading-[18px] hover:opacity-90 transition-all duration-300">
              Contact Us
            </button>
          </div>

          {/* Mobile Navigation Menu */}
          <nav className={`${menuOpen ? 'block fade-in' : 'hidden'} lg:hidden absolute top-full left-0 right-0 mt-4 bg-black/40 backdrop-blur-[12.5px] rounded-lg mx-8 border border-white/10 hover-glow`}
               style={{
            ...
            <div className="flex flex-col gap-4 p-6 stagger-delay">
              {menuItems.map((item, index) => (
                <a
                  key={index}
                  href={item.href}
                  className="fade-up text-left text-white font-poppins text-base font-medium hover:text-white/80 transition-all duration-300 py-2 relative group"
              ...
              <div className="pt-2 fade-up">
                <button className="hover-scale w-full flex px-6 py-4 justify-center items-center gap-2.5 rounded-[10px] border border-white/20 bg-nordek-button-gradient text-white font-poppins text-sm font-medium leading-[18px] hover:opacity-90 transition-all duration-300">
                  Contact Us
                </button>
              </div>
            </div>
